/*
    Creating a full text search library with limited features designed to use low resources while compiling an index for large corpora. 

    Details: 
    1. We are using 0 calculations on scoring and weighting documents, there is no boosting support. The purpose of this FTS is to create an accurate representation of a body of works
        NOT to put forward the best match. The huge assumption here is that users will be reading a large amount of the search results (i.e researchers looking over many docs/ collecting data) rather than looking at one or two results. 
        So - we dump all that extra overhead bc we don't plan to use it. 
    2. We create an index object right off the bat and we give the add and remove document ability, this alows for adding document in a pipeline - we assume the original doc does not need to be saved, 
        so when creating the index we can simply run a pipeline of loading in document - then adding it to the index, effectively decreasing browser RAM costs since the index will be far less ram than the original doc. 
    3. the pipeline for doc adding is as follows - Isolate and tokenize words -> stemmer -> stopper -> add to index

    4. The stopper is atm still unimagined, but will likely be a more passive approach of adding all the stopper words to our index with  a -1 entry to signify a stopper word. 

    5. Optional flag to store word indexes in the index data. - will literally only save the start index and lenght of token - assumption - the user will have original doc available to reference these indexes to find 
        the actual word. 
    6. We dont care about fields in our implementation so we just ask for key and text, later implementations might improve on this. 
*/
...
// default stemmer alg  - know dont need to redefine but 
...
export default class Index {
    ...
    add(id, document) {
        ...
        // TODO - we have to actually write word split by hand to retain indexes, I think just word split on " ", take note of indexes and lens here, then clean the tokens for the dict.
        // TODO - run tests to determine how clean we are making these tokens and what use cases we need to address - might even add optimization options in params
      ...
      /*  // we could single pass but lets do it here for now since we will be changing things with match indexe feature
        for (let word of newWords){
            if(this._index[word] === undefined){
                this._index[word] = [id]
            }
            else if (this._index[word] != -1){
                this._index[word].push(id);
            }
            // otherwise we fall thru for a stop word! 
        }
*/
        // okay skipping the match indexes for now
